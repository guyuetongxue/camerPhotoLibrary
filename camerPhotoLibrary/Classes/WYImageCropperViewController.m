//
//  WYImageCropperViewController.m
//  CapturePic
//
...
inline static float WYImageCropperViewControllerGetbottomHeightX() {
    float h = 0.0;
    if ([[UIApplication sharedApplication] statusBarFrame].size.height + 44 == 88) {
        h = 34;
    }
    return h;
}
...
@interface WYImageCropperViewController ()
...
@implementation WYImageCropperViewController
...
- (void)initView {
    self.showImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width-100, self.view.frame.size.height)];
...
- (void)initControlBtn {
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 50.0f - WYImageCropperViewControllerGetbottomHeightX(), 100, 50)];
    ...
    UIButton *confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, [UIScreen mainScreen].bounds.size.height - 50.0f - WYImageCropperViewControllerGetbottomHeightX(), 100, 50)];
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(WYImageCropperDelegate)]) {
